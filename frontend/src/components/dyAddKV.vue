<template>
  <div>
    <div class="flex justify-start pl-3">
      ...
      <a-button class="bg-blue-200 flex justify-center items-center hover:bg-blue-100" @click="addInput">
        <icon-park
            class="mr-1"
    ...
    <div v-for="(item, index) in inputs" :key="index" class="flex mt-2 px-3 justify-between">
      <div class="mt-1">
        <a-input v-model="item.key" :placeholder="props.keyPlaceholder" prefix="" suffix=""/>
      </div>
      <div class="mt-1">
        <a-input v-model="item.value" :placeholder="props.valuePlaceholder" prefix="" suffix=""/>
      </div>
      <div class="mt-1">
        <a-button v-if="inputs.length > 0" type="default" @click="removeInput(index)">移除</a-button>
      </div>
    </div>
