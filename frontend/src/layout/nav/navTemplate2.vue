...
const navWidth = ref<number>(14);
//图标div 宽高
const iconDivWH = ref<number>(12);
...
<template>
  <div class="flex bg-white justify-start items-start w-screen h-screen">
    ...
    <div
      ...
      :class="'w-' + navWidth"
      class="flex flex-col justify-center items-center h-full overflow-hidden text-gray-700 rounded"
    >
      ...
      <div
        ...
        :class="item.borderTop ? 'mt-2 border-t' : ''"
        class="flex flex-col items-center justify-center border-gray-300"
      >
        <router-link
          :class="
            [
              navStore.ActiveName === item.name && 'bg-gray-400',
              ,
              'w-' + iconDivWH,
              'h-' + iconDivWH,
            ].join(' ')
          "
          :to="item.path"
          class="flex items-center justify-center mt-1 rounded hover:bg-gray-300"
          href="#"
      ...
      <a
        :class="['w-' + iconDivWH, 'h-' + iconDivWH].join(' ')"
        class="flex items-center justify-center mt-auto bg-gray-200 hover:bg-gray-300"
        href="#"
      >
        <icon-park
          :size="iconSize"
